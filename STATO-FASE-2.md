...
- ✅ **Migrazione `middleware.ts` → `proxy.ts`** (chiuso nel rodaggio post-Fase 3, vedi `STATO-RODAGGIO.md`): rinominato il file e cambiato l'export da `auth: middleware` a `auth: proxy`. Build pulita, nessun warning, runtime Node funzionante.
- ✅ **Focus trap nelle modali admin** (chiuso nel rodaggio post-Fase 3): introdotto hook `useFocusTrap` in `src/hooks/useFocusTrap.ts` e applicato a tutte e 5 le modali admin con `role="dialog"` / `aria-modal="true"`. Resta aperto un audit a11y più ampio (form admin, screen reader) per Fase 4.
...
- ✅ **UI di pubblicazione/depubblicazione ristorante mancante** (chiuso nel rodaggio post-Fase 3): aggiunta sezione "Stato pubblicazione" in fondo alla tab Info con `ToggleField` + `ConfirmModal`. Server action `setRestaurantPublished` con `requireOwnership` e `revalidatePath` su admin + slug pubblico.
- ✅ **`prisma.config.ts` vs `package.json#prisma` — deprecation warning** (chiuso nel rodaggio post-Fase 3): rimosso il blocco `prisma` da `package.json` e aggiunto `migrations.seed: "tsx prisma/seed.ts"` in `prisma.config.ts`. Niente più warning a `prisma generate` / `db seed`.
